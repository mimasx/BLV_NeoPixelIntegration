...
enum  pattern { NONE, RAINBOW_CYCLE, THEATER_CHASE, COLOR_WIPE, SCANNER, PROGRESSBAR, PROGRESSBARFADE, FADE };
...
class NeoPatterns : public Adafruit_NeoPixel
{
        ...
        if((millis() - lastUpdate) > Interval) // time to update
        {
            ...
            switch(ActivePattern)
            {
                ...
                case PROGRESSBAR:
                    ProgressBarUpdate();
                    break;
                case PROGRESSBARFADE:
                    ProgressBarFadeUpdate();
                    break;
                case SCANNER:
                    ScannerUpdate();
                    break;
    ...
    { 
        for (int i = 1; i <= numPixels(); i++)
        {
            if (i < Progress)
            {
                 ...
                 //setPixelColor((numPixels()-i), Color(Red(Color2)*Index/16,Green(Color2)*Index/16,Blue(Color2)*Index/16));
            }
            else
            {
        ...
        setPixelColor((numPixels()-Progress),Color(Red(Color2)*Index/16,Green(Color2)*Index/16,Blue(Color2)*Index/16));
        show();
        Increment();
    }

    
    // Initialize for a ProgressBarFade
    void ProgressBarFade(uint32_t color1, uint32_t color2, uint8_t interval)
    {
        ActivePattern = PROGRESSBARFADE;
        Interval = interval;
        TotalSteps = numPixels();
        Color1 = color1;
        Color2 = color2;
    ...
    // Update the ProgressBarFade Pattern
    void ProgressBarFadeUpdate()
    { 
        for (int i = 1; i <= numPixels(); i++)
        {
            if ((i/Index<=1)&i<=Progress)
            {
              ...
              {
                setPixelColor(numPixels()-i, Color(Red(Color2)*0.1,Green(Color2)*0.1,Blue(Color2)*0.1));
              }
            }
            else if (i <= Progress)
            {
...
*/
  ...
  if(SerialReadError!=true){
    ...
    if(FloatTemp!=0){
      Printer.FractionPrinted=float(FloatTemp);
    }
  }
  else{
...
{   
  ...
  Serial.println("Duet port started");
    ...
    // Kick off a pattern
    Ring1.ProgressBarFade(Ring1.Color(25,25,25), Ring1.Color(255,0,0), 100);
    Ring2.Fade(Ring2.Color(25,25,25), Ring2.Color(0,255,0), 8, 100);
    Ring3.ProgressBarFade(Ring3.Color(25,25,25), Ring3.Color(255,0,0), 100);
}

int AmbientTemp;
void getAmbientTemp()
{
  //AmbientTemp=15; //nicer Display of ProgressBars. Best solution is to add a Ambient Temperature sensor.
  bool TempSensor = false; //Set TempSensor to True, When an Analog Sensor is attached to Arduino's A0.
  if(TempSensor){
    long a =1023 - analogRead(A0);
    //Adjust the Temp Calculation, if you use a different Sensor. I used the following Transostor: http://wiki.sunfounder.cc/index.php?title=Thermistor_Module
    AmbientTemp = 4090 /(log((1025.0 * 10 / a - 10) / 10) + 4090 / 298.0) - 273.0; 
  }
  else{
    AmbientTemp = 15;
  }
...
*/
    ...
    switch (Printer.Status)
    {
      ...
      case 9:
        ...
        break;
      default:
        Ring2.Color2 = Ring2.Color(25,25,25);
        break;
    }
